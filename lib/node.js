...
// detach all children from this node and return each, not linked to
// each other, in an array in same order.
Node.prototype.detachChildren = function() {
    let children = [];
    let nextchild = null;
    for (let child = this._firstChild; child; child = nextchild) {
        children.push(child);
        nextchild = child._next;
        child.unlink();
    }
    return children;
};
...
Node.testbools = function() {
    var names = [];
    for (let t in nodeNames) names.push(t);
    names.sort();
    for (let t of names) {
        const x = new Node(t);
        console.log('%s\t%s\t%s', t, x.isBlock() ? 'block' : ' ', x.isContainer() ? 'Container' : ' ');
    }
    throw new Error();
};
