...
function toc(node, entering) {
    this.cr();
    if (entering) {
        //SCG: console.log("toc(%s, %s)", node.onEnter, entering);
        let table = this.tochelper(node.getRoot(), node.maxdepth);
        //SCG: console.log(table);
        let currentDepth = 0;
...
function question(node, entering) {
    if (entering) {
        ...
        this.tag('/span');
        //SCG: console.log(node.level, this.lastQuestionLevel, this.questionNumbers);
...
HtmlRenderer.prototype.toc = toc;
HtmlRenderer.prototype.question = question;
HtmlRenderer.prototype.anchor = anchor;
